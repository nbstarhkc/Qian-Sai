(() => {
  ...
  const showDetail=()=>{document.body.classList.add('mobile-detail');if(window.innerWidth<=760)window.scrollTo({top:0,behavior:'instant'})};
  let comparison=[];
  ...
  function renderDetail(){
    ...
    if(!t){$('#topic-detail').innerHTML=`<div class="empty">${icon('search-x')}<h3>未找到匹配选题</h3><p>当前企业、技术方向或关键词没有对应结果。</p></div>`;refreshIcons();return}
    const v=vendor(t.vendor);
    $('#topic-detail').innerHTML=`
      <button class="back-list" data-back-list>${icon('arrow-left')}赛题目录</button>
      <div class="detail-top"><div class="detail-title"><p class="detail-eyebrow"><span class="vendor-dot" style="--vendor:${v.color}"></span>${v.name}<span>${t.number}</span><span class="topic-kind">${t.type}</span></p><h2>${esc(t.name)}</h2>${t.nameNote?`<p class="name-note">${esc(t.nameNote)}</p>`:''}</div><a class="cover-link" href="${pdf(t)}" target="_blank" rel="noopener" title="${v.name}官方指南"><img src="assets/${v.id}-cover.webp" alt="${v.name} 2026 官方选题指南封面" width="67" height="95"></a></div>
      <p class="summary">${esc(t.summary)}</p>
      <div class="detail-tools"><button class="primary ${comparison.includes(t.id)?'selected':''}" data-toggle-compare="${t.id}">${icon(comparison.includes(t.id)?'check':'columns-3')}${comparison.includes(t.id)?'已加入对比':'加入对比'}</button><a class="secondary" href="fulltext.html#${t.vendor}-${t.page}">${icon('book-open-text')}逐页全文</a><button class="icon-button" data-print aria-label="打印此题" data-tooltip="打印此题">${icon('printer')}</button><a class="source-ref" href="${pdf(t)}" target="_blank" rel="noopener">PDF · 第 ${t.page}${t.end!==t.page?'–'+t.end:''} 页${icon('arrow-up-right')}</a></div>
      <div class="hardware"><span>${icon('cpu')}平台 / 工具</span><p>${esc(t.hardware)}</p></div>
      <section class="flow-section"><div class="section-kicker">${icon('workflow')}任务链路 · 归纳</div><ol class="flow">${t.flow.map((s,i)=>`<li><span class="step">0${i+1}</span><span>${esc(s)}</span>${i<t.flow.length-1?'<i class="flow-arrow" data-lucide="chevron-right"></i>':''}</li>`).join('')}</ol></section>
      <aside class="caution"><h3>${icon('triangle-alert')}关键限制与易混淆点</h3>${list(t.cautions)}</aside>
      ...
      <aside class="advice"><h3>${icon('lightbulb')}备赛建议 · 非官方要求</h3><p>${esc(t.advice)}</p></aside>
  ...
  function openTopic(id,scroll=true){reset();current=id;renderList();setView('topics');showDetail();if(scroll&&window.innerWidth>760)$('#topic-detail').scrollIntoView({block:'start',behavior:'instant'})}
  ...
  function renderComparison(){
    ...
    if(!comparison.length){$('#comparison').innerHTML=`<div class="compare-empty">${icon('columns-3')}<h3>尚未选定对比项</h3><button class="secondary" data-view="topics">${icon('layout-list')}浏览赛题</button></div>`;refreshIcons();return}
  ...
  function renderSources(){
    $('#source-grid').innerHTML=vendors.map(v=>`<article class="source-card"><img src="assets/${v.id}-cover.webp" alt="${v.name}官方指南封面" width="70" height="99" loading="lazy"><div><h3><span class="vendor-dot" style="--vendor:${v.color}"></span> ${v.name}</h3><p class="meta">${v.pages} 页 · ${topics.filter(t=>t.vendor===v.id).length} 项 · 文件版本 ${v.date}</p><p>${esc(v.boards)}</p><p class="support">技术交流：${esc(v.support)}</p><div class="source-actions"><a class="secondary" href="sources/${v.id}.pdf" target="_blank" rel="noopener">${icon('file-text')}PDF</a><a class="secondary" href="fulltext.html#${v.id}-1">${icon('book-open')}逐页全文</a><a class="secondary" href="fulltext/${v.id}.txt" download>${icon('download')}TXT</a></div></div></article>`).join('');
    refreshIcons();
  }
  $('#category').insertAdjacentHTML('beforeend',tags.map(x=>`<option>${x}</option>`).join(''));
  $('#vendor-mobile').insertAdjacentHTML('beforeend',vendors.map(v=>`<option value="${v.id}">${v.name}</option>`).join(''));
  $('#vendor-mobile').addEventListener('change',()=>{selectedVendor=$('#vendor-mobile').value;renderVendors();renderList()});
  ...
  document.addEventListener('click',e=>{
    ...
    if(b.dataset.vendor){selectedVendor=b.dataset.vendor;renderVendors();renderList();setView('topics');if(window.innerWidth>760)$('#view-topics').scrollIntoView({block:'start',behavior:'instant'})}
    if(b.dataset.topic){current=b.dataset.topic;renderList();history.replaceState(null,'','#'+current);showDetail()}
    if(b.hasAttribute('data-back-list')){document.body.classList.remove('mobile-detail');history.replaceState(null,'','#topics');$('#topic-list').scrollIntoView({block:'start',behavior:'instant'})}
  ...
  function readHash(){let value='';try{value=decodeURIComponent(location.hash.slice(1))}catch{}if(topic(value)){reset();current=value;renderList();setView('topics',false);showDetail()}else setView(value||'topics',false)}
